...
class ViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        guard let dvc = segue.destination as? SecondViewController else { return }
            dvc.info = "Создать приложение которое бы помогало людям выполнять все предписания врача. Например напоминать о приеме лекарств и помгоать соблядать последовательность приема"
            dvc.pageName = "Идеи"}
